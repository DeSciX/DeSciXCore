...
import {
    ...
    GROUNDING_VERDICTS, GROUNDING_FLAG_FIELD_NAMES, GROUNDING_FLAG_DISPOSITIONS,
    GROUNDING_FLAG_RESOLUTION_REQUIRED_FOR,
...
export const NATIVE_MCP_TOOLS = Object.freeze([
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                thinking_budget: { type: 'number', description: 'Thinking-token budget for THIS call: -1 dynamic, 0 off, N a fixed cap. The number is mapped onto the provider\'s thinking LEVEL, and which levels exist is a property of the MODEL: on a model whose vocabulary is {high, low} (measured: gemini-2.5-flash, gemini-2.5-pro) there is no "off", so 0 is REFUSED naming the model and its allowed values rather than sent onward. Thinking is also spent from max_output_tokens, so a cap too small to fit it is refused too. It is the request tier\'s thinking statement and wins over whatever the chain resolved. Omit to take the thinking setting from the same tier that supplied the model. ' + MODEL_THINKING_CHAIN },
    ...
    },
    {
        ...
        description: 'Use your app like a database: store/replace structured records with custom metadata in the app data plane (Firestore-backed, strongly consistent — NOT the Pinecone KB). Supply a human-meaningful file_id (grouping key) + optional chunk_idx (default 0); the composite id is built for you. Custom string/number/boolean/string[] fields become filterable metadata for app_records_query (a string[] field is filtered with ' + ARRAY_FILTER_OPERATORS[0] + ', not with equality). Pass mode:"create" for an atomic first-claim-wins conditional create (answers claimed:true / claimed:false + current_holder_hint instead of overwriting). Records under a "lease-" key are holder-verified server-side. TIME: every record is stamped with a server-authoritative received_at — the store\'s own UTC clock at the moment it accepted the write. It is server-owned: a client-supplied received_at is stripped and replaced, so do not send one. Any created_at you send is ordinary client-asserted metadata with no more authority than text — the store neither validates nor corrects it. CONTENT HASH: every put returns content_sha256 — the sha256 of the record\'s STORED TEXT ONLY (the `text` field), hex, so you can verify delivery by recomputing it yourself over the text you sent. A record with NO text field returns null, which is deliberately NOT the hash of the empty string: "no text" and "empty text" are different facts. A single-record put returns the scalar content_sha256; every put also returns content_sha256_by_file_id keyed by file_id. IDEMPOTENCE: a SINGLE-RECORD upsert whose whole stored payload is already byte-identical performs NO WRITE and answers unchanged:true with unchanged_file_ids — so received_at does NOT advance and a retry cannot manufacture a fake "the store accepted something" event. The comparison is the WHOLE record, not just the text: a put that changes only metadata still writes, because deciding from the text hash alone would silently drop it. THREE EXCEPTIONS, where the WRITE ITSELF IS THE SIGNAL and an identical repeat therefore STILL writes and STILL advances received_at: keys prefixed "lease-", "heartbeat-" and "watermark-". received_at remains "when the store last ACCEPTED a write" for those classes, which is what liveness and the inbox time-floor read. A MULTI-RECORD put is never deduplicated — the bulk path writes blind by design so a KB sync writing thousands of chunks does not pay for a read-before-write.',
        mutating: true,
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Write the GROUNDING RECEIPT for a contract and an identity — the served proof that this identity performed the contract\'s declared KB reads and checked the answers against the code. It is what a guard reads before it will let a doer edit a file, which is the whole point: grounding becomes a record written on an observed event rather than an obligation an agent has to remember. The key is DERIVED as "' + KEY_GROUNDING + '<contract_key>-<identity>[-<phase>]" with the contract key and identity taken BYTE-EXACT (`contract_key` is the full stored key, so grounding-contract-ws-x-DOER is correct and its doubled-looking "contract-" is the contract\'s own name, not a mistake); `phase` is an OPTIONAL third segment for work that grounds more than once, and without it a second grounding would overwrite the first. A client-supplied file_id/key is REFUSED, as is any clock (occurred_at/received_at/created_at) — a caller-stamped grounding could claim it happened at a time no write did, which is precisely the audit trail this record exists to be. `reads` is REQUIRED and NON-EMPTY, and every entry carries a `kind` (' + GROUNDING_READ_KINDS.join(' | ') + ') that SELECTS the fields it is held to: kind "kb" = a corpus question carrying ' + GROUNDING_KB_READ_FIELD_NAMES.join(', ') + '; kind "role" = a served role-record read carrying ' + GROUNDING_ROLE_READ_FIELD_NAMES.join(', ') + ' INSTEAD of app/knowledgebase_names/question, because a record fetch has no sources to count. At least one "' + GROUNDING_REQUIRED_READ_KIND + '" read is REQUIRED — a receipt made only of role reads proves the doer read its own job description, not the contract\'s corpus. A kb READ WITH sources_count 0 IS REFUSED BY NAME (FABRIC_GROUNDING_READ_UNSOURCED): an answer that cited no sources is the model speaking from its own weights, which is the ungrounded answer the gate exists to catch — recording it faithfully would still produce a receipt a guard lets through. `verification` is REQUIRED and NON-EMPTY, each entry carrying ' + GROUNDING_VERIFICATION_FIELD_NAMES.join(', ') + ' — a grounding with no verified claim is not a grounding, and `file_symbol` (path::symbol) is required so the verifier can re-run the check instead of taking the doer\'s word. `verdict` is a CLOSED ENUM (' + GROUNDING_VERDICTS.join(', ') + ') because a guard branches on it and cannot parse English; UNDETERMINED exists so a check that could not settle has somewhere honest to go. `flags` is REQUIRED and MAY BE EMPTY, and every entry is an OBJECT carrying ' + GROUNDING_FLAG_FIELD_NAMES.join(', ') + ' — a BARE STRING IS REFUSED BY INDEX rather than wrapped, since wrapping would invent a disposition you never claimed; disposition is one of ' + GROUNDING_FLAG_DISPOSITIONS.join(', ') + ' and ' + GROUNDING_FLAG_RESOLUTION_REQUIRED_FOR.join('/') + ' REQUIRE a non-empty `resolution`. "I found no flags" is a claim a doer must make explicitly, and an absent array is indistinguishable from a doer who never looked. The `contract_key` must RESOLVE to an existing contract record (FABRIC_GROUNDING_CONTRACT_UNKNOWN) and that contract must itself pass the contract-body rules (FABRIC_GROUNDING_CONTRACT_MALFORMED, raised by running the SAME validator fabric_contract_put runs) — a doer cannot ground against a contract that never stated what to read. Call fabric_vocabulary for the published field tables and refusal codes rather than hand-keeping a copy.',
        mutating: true,
        inputSchema: {
            type: 'object',
            properties: {
                contract_key: { type: 'string', description: 'The FULL stored key of the contract being grounded against (starting "' + KEY_CONTRACT + '", exactly as the contract record\'s own contract_key field carries it). It must resolve to an existing contract record, and that contract must carry knowledge.reads[] and principles_carried. Nothing is stripped: it goes into the grounding key whole.' },
                ...
                reads: {
                    type: 'array',
                    description: 'REQUIRED, non-empty, and at least one entry of kind "' + GROUNDING_REQUIRED_READ_KIND + '". Each entry carries `kind` (' + GROUNDING_READ_KINDS.join(' | ') + '), which selects its field table: kb → ' + GROUNDING_KB_READ_FIELD_NAMES.join(', ') + ' (`question` is passed verbatim as ask_question_to_app.user_input; sources_count 0 is REFUSED); role → ' + GROUNDING_ROLE_READ_FIELD_NAMES.join(', ') + '.',
                    items: { type: 'object' },
                },
                verification: {
                    type: 'array',
                    description: 'REQUIRED, non-empty. One entry per KB claim checked against the code. Fields: ' + GROUNDING_VERIFICATION_FIELD_NAMES.join(', ') + '. `verdict` must be one of ' + GROUNDING_VERDICTS.join(', ') + ' — a closed enum, because a guard branches on it.',
                    items: { type: 'object' },
                },
                flags: {
                    type: 'array',
                    description: 'REQUIRED, MAY BE EMPTY. The flags raised to the hiring party before work began, as OBJECTS carrying ' + GROUNDING_FLAG_FIELD_NAMES.join(', ') + '. A bare string is REFUSED by index. `disposition` is one of ' + GROUNDING_FLAG_DISPOSITIONS.join(', ') + '; ' + GROUNDING_FLAG_RESOLUTION_REQUIRED_FOR.join(' and ') + ' require a non-empty `resolution`. Empty array means "I looked and found none" — an explicit claim, not an omission.',
                    items: { type: 'object' },
                },
                phase: { type: 'string', description: 'OPTIONAL. A slug naming the phase of work this receipt covers, appended as a third key segment ("' + KEY_GROUNDING + '<contract_key>-<identity>-<phase>"). Send it when one identity grounds a contract more than once; without it the second receipt overwrites the first.' },
