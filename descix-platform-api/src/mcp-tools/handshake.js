...
export const MCP_HANDSHAKE_INSTRUCTIONS = [
    ...
    "1. tell_me_how({ question, scope }) — capability discovery; scope:'bootstrap' is the canonical first call (platform summary, Evidence Contract, caller context, credit balance, essential tool schemas). Pass the user's own words as `question` even when you set a scope, so intents like onboarding are detected.",
...
export const PLATFORM_BOOTSTRAP_SUMMARY = [
    ...
    "The platform is headless-MCP-first: there is no dashboard to learn. Apps and services are discovered at runtime rather than from hardcoded endpoints, so you ask what exists and then invoke what you were told.",
    "Your operating contract as an external consumer: (1) discover capability with tell_me_how(question, scope) — it searches the platform's vectorized service documentation, and you should pass the user's own words as question even when you set a scope, so intents like onboarding are detected; (2) invoke what it names with execute_remote_command({ command, params }) — only a small discovery-core set is advertised at handshake, and everything else is discovered through it. On claude.ai a tool may be deferred until first use: if a call returns \"not loaded yet,\" load it with your tool-search and retry — that is expected, not an error. (3) fetch_my_purchases lists what the caller owns; (4) RAG queries (ask_question_to_app, query_knowledge_base) draw on a shared USD AI-credit balance; get_credit_balance shows it.",
